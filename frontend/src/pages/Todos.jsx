...
export default function Todos() {
  ...
  const loadData = async () => {
    try {
      const [todosData, statsData, colsData] = await Promise.all([
        api(
          "GET",
          `/todos?sortBy=${sort}${filter !== "all" ? `&status=${filter}` : ""}${priority ? `&priority=${priority}` : ""}${searchQuery ? `&search=${searchQuery}` : ""}`,
        ),
  ...
  const displayedTodos = collectionId
    ? todos.filter((t) => t.collectionId === collectionId)
    : todos;
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-surface border border-border rounded-xl p-5 relative overflow-hidden before:absolute before:top-0 before:left-0 before:right-0 before:h-0.5 before:bg-gradient-to-r before:from-accent before:to-accent2">
          <div className="font-syne text-3xl font-extrabold mb-1">
            {stats.total}
          </div>
          <div className="text-muted text-xs">Total Tasks</div>
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-3xl opacity-15">
            📋
          </span>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <button
          className={`px-3 py-1.5 rounded-full border text-xs transition-colors ${filter === "all" ? "bg-accent border-accent text-white" : "border-border text-muted hover:bg-surface hover:text-text"}`}
          onClick={() => setFilter("all")}
        >
          All
        </button>
        <button
          className={`px-3 py-1.5 rounded-full border text-xs transition-colors ${filter === "completed" ? "bg-accent border-accent text-white" : "border-border text-muted hover:bg-surface hover:text-text"}`}
        ...
        <select
          className="px-3 py-1.5 bg-surface border border-border rounded-xl text-sm outline-none cursor-pointer text-text"
          value={priority}
      ...
      <div className="flex flex-col gap-3">
        {displayedTodos.length === 0 ? (
          ...
          displayedTodos.map((t) => {
            ...
            const isOverdue =
              t.dueDate && new Date(t.dueDate) < new Date() && !isCompleted;

            return (
              <div
                key={t._id}
                className={`group bg-surface border border-border rounded-xl p-4 flex items-start gap-4 transition-all hover:border-[#ffffff20] hover:shadow-[0_4px_24px_#00000066] relative ${isCompleted ? "opacity-50" : ""}`}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`text-[0.95rem] font-medium mb-1 ${isCompleted ? "line-through text-muted" : ""}`}
                  >
                    {t.title}
                  </div>
                  {t.description && (
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-1">
                    <span
                      className={`px-2 py-0.5 rounded-full text-[0.7rem] font-semibold ${t.priority === "high" ? "bg-[#f8717122] text-high" : t.priority === "medium" ? "bg-[#fbbf2422] text-medium" : "bg-[#34d39922] text-low"}`}
                    >
                      {t.priority}
                    </span>
                    ...
                    {t.dueDate && (
                      <span
                        className={`text-[0.75rem] flex items-center gap-1 ${isOverdue ? "text-danger" : "text-muted"}`}
                      >
                        📅{" "}
                    ...
                    {col && (
                      <span
                        className="bg-surface2 rounded-md px-2 py-0.5 text-[0.7rem]"
                    ...
                    {(t.tags || []).map((tag) => (
                      <span
                        key={tag}
                        className="bg-surface2 rounded-md px-2 py-0.5 text-[0.7rem] text-muted"
                      >
                        #{tag}
                      </span>
                    ))}
                ...
                <div className="flex gap-2 opacity-0 transition-opacity group-hover:opacity-100 absolute top-4 right-4">
                  <button className="tiny-btn" onClick={() => openModal(t)}>
                    ✏️
                  </button>
                  <button
          ...
          <div className="bg-surface border border-border rounded-[20px] p-8 w-full max-w-[500px] max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={formik.handleSubmit}>
              ...
              <div className="flex gap-3">
                ...
                >
                  {formik.isSubmitting ? "Saving..." : "Save Todo"}
                </button>
              </div>
            </form>
          </div>
