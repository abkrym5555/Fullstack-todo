  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div>
          <h2 className="font-syne text-3xl font-bold bg-gradient-to-br from-accent to-accent2 bg-clip-text text-transparent">
            User Feedback
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {feedbacks.length === 0 ? (
          ...
          feedbacks.map((f) => (
            <div
              key={f._id}
              className="bg-surface border border-border rounded-xl p-5 hover:border-[#ffffff20] hover:shadow-[0_4px24px#00000066] transition-all"
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <span
                      key={i}
                      className={`text-sm ${i < f.rating ? "text-accent" : "text-muted opacity-30"}`}
                    >
                      ★
                    </span>
                  ))}
                  ...
                  {[5, 4, 3, 2, 1].map((star) => (
                    <button
                      ...
                      onMouseLeave={(e) => {
                        const buttons = e.currentTarget.parentNode.children;
                        Array.from(buttons).forEach((b, i) => {
                          if (Number(formik.values.rating) < 5 - i)
