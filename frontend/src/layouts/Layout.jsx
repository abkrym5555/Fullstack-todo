    ...
    <div className="flex min-h-screen">
      ...
      <aside className={`fixed md:static left-0 top-0 w-[260px] h-screen bg-surface border-r border-border flex flex-col z-[100] transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          <div className="mb-6">
            <div className="text-[0.7rem] font-semibold tracking-widest text-muted uppercase px-3 py-1 mb-2">Workspace</div>
        ...
        <div className="p-4 border-t border-border">
          <div className="flex items-center gap-3 p-3 bg-surface2 rounded-xl cursor-pointer hover:bg-surface transition-colors" onClick={() => navigate('/profile')}>
            {currentUser?.profilePicture ? (
              <img src={currentUser.profilePicture} alt="Avatar" className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
            ) : (
          ...
          <button onClick={logout} className="w-full mt-2 text-xs text-muted hover:text-danger py-2 text-left px-3 transition-colors">Sign out</button>
        </div>
      </aside>
        ...
        <div className="flex items-center justify-between p-5 md:px-8 border-b border-border bg-bg sticky top-0 z-50">
          <div className="flex items-center gap-4">
            ...
            <div className="font-syne text-xl font-bold capitalize">
              {location.pathname === '/' ? 'All Todos' : location.pathname.substring(1)}
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/?new=true')} className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-br from-accent to-[#5b4fd4] rounded-xl text-white font-sans text-sm cursor-pointer hover:opacity-90 whitespace-nowrap">
              ＋ New Todo
            </button>
          </div>
        </div>
